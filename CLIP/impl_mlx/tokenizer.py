...
class CLIPTokenizer:
    ...
    def __init__(self, bpe_ranks, vocab):
        ...
        self.pat = regex.compile(
            r"""<\|startoftext|>|<\|endoftext|>|'s|'t|'re|'ve|'m|'ll|'d|[\p{L}]+|\p{N}|""",
            regex.IGNORECASE
        )
        self._cache = {
            self.bos: self.bos,
    ...
    def eos(self):
        """ end of sequence 
        "[bos] A photo of a cat sitting on [eos]"
        """
        return "<|endoftext|>"
    ...
    def bpe(self, text):
        ...
        # if none was merged we are done.
    ...
    def tokenize(self, text, prepend_bos=True, append_eos=True) -> mx.array:
        if isinstance(text, list):
            return mx.array([
                self.tokenize(t, prepend_bos, append_eos)
                for t in text
            ])
        
        # lower case, cleanup and split
